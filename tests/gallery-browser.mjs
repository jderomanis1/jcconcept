...
const themeCount=theme=>manifest.photos.filter(x=>x.themes.includes(theme)).length;
...
for(const [type,width,height]of [[chromium,320,740],[chromium,390,844],[chromium,768,1024],[chromium,1101,900],[chromium,1440,1000],[webkit,390,844]]){
 ...
 try{
  ...
  assert.equal(await p.locator('#cleaning .cleaning-card').count(),3);
  assert.equal(await p.locator('#cleaning a[href*="setmore.com"]').count(),0);
  const referral=p.locator('#cleaning a[data-service="Cleaning referral"]');assert.equal(await referral.getAttribute('href'),'#contact');
  assert.match(await p.locator('#cleaning').innerText(),/trusted family cleaning partner/i);
  ...
  for(const theme of ['interiors','outdoors','preparation','workshop','all']){await p.locator(`.gallery-filters [data-theme="${theme}"]`).click();const expected=theme==='all'?count:themeCount(theme);assert(expected>=3||theme==='all');assert.equal(await p.locator('.gallery-card:not([hidden])').count(),expected);}
  ...
  if(width<500)await first.tap();else await first.click();await readyImage(p);assert(await p.locator('#photo-dialog').evaluate(e=>e.open));assert(await p.locator('#photo-close').evaluate(e=>e===document.activeElement));assert.match(await p.locator('#viewer-count').innerText(),new RegExp(`1 / ${themeCount('outdoors')}`));
  if(width===390||width===1440){await audit(p,`${name}-viewer`);await p.screenshot({path:`qa-artifacts/${name}-photo-viewer.png`});}
  const title=await p.locator('#viewer-title').innerText();await p.locator('#photo-next').click();await readyImage(p);assert.notEqual(await p.locator('#viewer-title').innerText(),title);await p.keyboard.press('ArrowLeft');await readyImage(p);assert.equal(await p.locator('#viewer-title').innerText(),title);await p.locator('#photo-prev').click();await readyImage(p);assert.match(await p.locator('#viewer-count').innerText(),new RegExp(`${themeCount('outdoors')} / ${themeCount('outdoors')}`));
  ...
  await p.goto(url('gallery.html?theme=workshop#photo=workshop-shelving'));await readyImage(p);assert.match(await p.locator('#viewer-count').innerText(),new RegExp(`/ ${themeCount('workshop')}$`));assert(!(await p.locator('#photo-next').isDisabled()));assert(!(await p.locator('#photo-prev').isDisabled()));await p.locator('#photo-close').click();await p.waitForFunction(()=>!document.querySelector('#photo-dialog').open);assert.equal(new URL(p.url()).hash,'');
  ...
  assert.deepEqual(errors,[],`${name} page errors`);assert.deepEqual(http,[],`${name} HTTP errors`);assert.deepEqual(outside,[],`${name} unexpected external requests`);assert.deepEqual(posts,[],`${name} unexpected submissions`);results.push({name,result:'PASS',photos:count,themeMinimum:3,filters:true,keyboard:true,history:true,exits:true,cleaning:true});console.log('PASS',name);
